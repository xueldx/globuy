...
```bash
uv run uvicorn app.presentation.server:app --host 0.0.0.0 --port 8310
```
...
浏览器访问 `http://localhost:5180`。后端默认地址为 `http://localhost:8310`，需要覆盖时设置 `VITE_API_BASE`。
...
uv run python scripts/loadtest.py --base-url http://localhost:8310 --stages 5,10,20
```
